...
function InitHeaderLAS_BCAL, versionMajor=versionMajor, versionMinor=versionMinor
...
if (N_Elements(versionMajor) && N_Elements(versionMior)) then begin
  
     ; If LAS 1.3, add the field for Start of Waveform Data Packet Record

if (versionMajor eq 1) && (versionMinor eq 3) then header = {header13, inherits header12, waveStart:0ULL} 

    ; If LAS 1.4, define the header differently here

if (versionMajor eq 1) && (versionMinor eq 4) then begin
header = {header14, $
    signature       : byte('LASF'), $               ; File signature
    fileSource      : 0US,  $                       ; File source ID
    reserved        : 0US,  $                       ; Reserved
    ...
    year            : 0US,    $                     ; File creation year
    headerSize      : 227US,  $                     ; Header size
    dataOffset      : 227ULL, $                      ; Offset to point data
    nRecords        : 0UL,   $                      ; Number of variable length records
    pointFormat     : 0B,    $                      ; Point data format ID
    pointLength     : 20US,   $                     ; Point data record length
    nPoints         : 0ULL,   $                      ; Number of point records
    nReturns        : ulon64arr(15), $                 ; Number of points by return
    xScale          : 0D, $                         ; X scale factor
    yScale          : 0D, $                         ; Y scale factor
    zScale          : 0D, $                         ; Z scale factor
    xOffset         : 0D, $                         ; X offset
    ...
    zMax            : 0D, $                         ; Max Z
    zMin            : 0D,  $                         ; Min Z
    waveStart   : 0ULL  $                         ; Start of Waveform Data Packet Record
}
endif
