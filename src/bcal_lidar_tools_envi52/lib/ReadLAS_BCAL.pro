...
; CALLING SEQUENCE:
;
;       ReadLAS_BCAL, inputFile, header, data, records=records, check=check, projection=projection, nodata=nodata, assoclun=assoclun, versionMajor=versionMajor, versionMinor=versionMinor
;
; RETURN VALUE:
;
...
;       of the variable length records.  If no records exist, a value of !NULL is returned.
;
...
;       embedded in the file.  If no information exists, a value of !NULL is returned.
;
...
;
;       Set the versionMajor and versionMinor keywords to return the LAS version information.
;
; DEPENDENCIES:
;
;       InitHeaderLAS_BCAL
...
;       Modified to handle LAS 1.3 and 1.4, April 2016
;
;###########################################################################
;
; LICENSE
...
pro ReadLAS_bcal, inputFile, header, data, records=records, check=check, projection=projection, $
             noData=noData, assocLun=assocLun, versionMajor=versionMajor, versionMinor=versionMinor

compile_opt idl2, logical_predicate

    ; Create the header structure

commonHeader = ReadCommonHeaderLAS_BCAL(inputFile)
versionMajor = commonHeader.versionMajor
versionMinor = commonHeader.versionMinor
header = InitHeaderLAS_BCAL(versionMajor=versionMajor, versionMinor=versionMinor)
...
point_lun, inputLun, header.headerSize
    
if header.nRecords then begin

        ; Define and read variable length records

    records = replicate(InitRecordLAS_BCAL($
      versionMajor=versionMajor, versionMinor=versionMinor), header.nRecords)

    for a=0,header.nRecords-1 do begin

        tempRecord = InitRecordLAS_BCAL($
          versionMajor=versionMajor, versionMinor=versionMinor, /noData)
...
if header.dataOffset ne (fInfo.size - header.nPoints * header.pointLength) then begin
    ...
    print, 'header.nPoints value is incorrect.  Fixing...'
    oldVal = header.nPoints
    header.nPoints = Long((fInfo.size - header.dataOffset) / header.pointLength)
    print, 'header.nPoints ', oldVal, ' is changed to ',  header.nPoints
