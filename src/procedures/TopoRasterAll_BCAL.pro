...
products = {allMin   :{title:'Elevation - Minimum',      points:1, index:-1, doIt:0}, $
            allMean   :{title:'Elevation - Mean',        points:1, index:-1, doIt:0}, $
            allMax   :{title:'Elevation - Maximum',      points:1, index:-1, doIt:0}, $
            allRough  :{title:'Absolute Roughness',      points:2, index:-1, doIt:0}, $
            allLRough :{title:'Local Roughness',         points:3, index:-1, doIt:0}, $
            allSlope  :{title:'Slope (degrees)',         points:3, index:-1, doIt:0}, $
            allAspect :{title:'Aspect (degrees from N)', points:3, index:-1, doIt:0}, $
...
gridBase = widget_auto_base(title='Topographic Products (All returns)')
...
if products.allSlope.doIt or products.allAspect.doIt then needall = 1 else needall = 0
...
;                            + iDim * jDim * ((max(prodIndex[nall:nProducts-1])) and (pData.source gt (50./header.zScale))) $
            ...
            if doPixel[i] then begin
                ...
                if needOne then begin
                    ...
                    while (index[0] eq -1L) do index = GetIndex_BCAL(i,j,iDim,jDim,arrayIndex,++factor)
                        ...
                        if products.allMin.doIt then $
                         dataArray[i,k,products.allMin.index] = min(pData[index].elev) $
                                                                                            * header.zScale + header.zOffset
                        if products.allMean.doIt then $
                         dataArray[i,k,products.allMean.index] = mean(pData[index].elev) $
                                                                                            * header.zScale + header.zOffset
                        if products.allMax.doIt then $
                         dataArray[i,k,products.allMax.index] = max(pData[index].elev) $
                                                                                            * header.zScale + header.zOffset
                        
                        if products.allDen.doIt then begin
                            dummy = where(pData[index].source eq 0, groundCount)
                            dataArray[i,k,products.allDen.index] = groundCount / grid^2
                        endif
                ...
                if needTwo then begin
                    ...
                    if (n_elements(index) ge 2) then begin
                        ...
                        if products.allRough.doIt then $
                         dataArray[i,k,products.allRough.index]  = stddev(pData[index].elev) $
                                                                      * header.zScale
                ...
                if needThree then begin
                    ...
                    while (doInterp and (n_elements(index) lt 6)) do index = GetIndex_BCAL(i,j,iDim,jDim,arrayIndex,++factor)
                        ...
                        ; If at least three data points are found and a all earth product is needed, begin.

                    if (n_elements(index) ge 6 and needall) then begin

                            ; Compute the slope using the all earth data points

                        allSlope = regress(transpose([[pData[index].east  * header.xScale], $
                                                       [pData[index].north * header.yScale]]), $
                                                       (pData[index].elev) * header.zScale, $
                                                       const=allConst, status=allStat)
                        ...
                        while (doInterp and ((allStat gt 0) or (min(finite(allSlope)) eq 0))) do begin
                        ...
                        if ((allStat eq 0) and (min(finite(allSlope)) eq 1)) then begin

                            if products.allSlope.doIt then $
                             dataArray[i,k,products.allSlope.index]  = atan(sqrt(total(allSlope^2))) * !radeg
                            
                            if products.allAspect.doIt then $
                             dataArray[i,k,products.allAspect.index] = atan(allSlope[0,0],allSlope[0,1]) * !radeg $
                                                                      + ((allSlope[0,0] lt 0) * 360)
                            if products.allTrasp.doIt then begin
                             
                             aspect = atan(allSlope[0,0],allSlope[0,1]) * !radeg $
                                                                      + ((allSlope[0,0] lt 0) * 360)
                             dataArray[i,k,products.allTrasp.index] = (1 - cos((!pi/180)*(aspect-30)))/2
                            
                            endif
                            
                            if products.allSlpCosAsp.doIt then begin
                             
                             dataArray[i,k,products.allSlpCosAsp.index] = sqrt(total(allSlope^2)) * cos(atan(allSlope[0,0],allSlope[0,1]) +  ((allSlope[0,0] lt 0) * 360))
                            
                            endif
                            
                            if products.allSlpSinAsp.doIt then begin
                             
                             dataArray[i,k,products.allSlpSinAsp.index] = sqrt(total(allSlope^2)) * sin(atan(allSlope[0,0],allSlope[0,1]) +  ((allSlope[0,0] lt 0) * 360))
                            
                            endif
                                                                      
                            if products.allLRough.doIt then begin

                                allDev = (pData[index].elev)  * header.zScale - (allConst $
                                        + pData[index].east  * header.xScale * allSlope[0] $
                                        + pData[index].north * header.yScale * allSlope[1])

                                dataArray[i,k,products.allLRough.index] = stddev(allDev)
