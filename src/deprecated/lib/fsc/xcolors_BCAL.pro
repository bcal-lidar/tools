...
;       23 April 2010. Renamed all procedures and functions with _BCAL suffix. RS
;-      23 April 2010. Updated older/legacy IDL codes. RS
;
;###########################################################################
;
; LICENSE
...
Function XColors_Congrid_BCAL, arr, x, y, z, INTERP=int, MINUS_ONE=m1, CUBIC = cubic
compile_opt idl2
...
PRO XColors_NotifyObj__Define_BCAL
compile_opt idl2
...
IF info.reverse EQ 0 THEN BEGIN
   r[info.currentbottom:info.currenttop] = XColors_Congrid_BCAL(red[index], number, /Minus_One)
   g[info.currentbottom:info.currenttop] = XColors_Congrid_BCAL(green[index], number, /Minus_One)
   b[info.currentbottom:info.currenttop] = XColors_Congrid_BCAL(blue[index], number, /Minus_One)
ENDIF ELSE BEGIN
   r[info.currentbottom:info.currenttop] = $
      Reverse(XColors_Congrid_BCAL(red[index], number, /Minus_One))
   g[info.currentbottom:info.currenttop] = $
      Reverse(XColors_Congrid_BCAL(green[index], number, /Minus_One))
   b[info.currentbottom:info.currenttop] = $
      Reverse(XColors_Congrid_BCAL(blue[index], number, /Minus_One))
ENDELSE
...
PRO XCOLORS_TOP_SLIDER_BCAL, event
compile_opt idl2
...
PRO XCOLORS_BOTTOM_SLIDER_BCAL, event
compile_opt idl2
   ; Get the info structure from storage location.
...
PRO XCOLORS_GAMMA_SLIDER_BCAL, event
compile_opt idl2
...
PRO XCOLORS_COLORTABLE_BCAL, event
compile_opt idl2
...
PRO XCOLORS_PROTECT_COLORS_BCAL, event
compile_opt idl2
...
XColors_Set_BCAL, info.cancelStruct
Widget_Control, event.top, Set_UValue=info, /No_Copy
...
PRO XCOLORS_DISMISS_BCAL, event
compile_opt idl2
...
PRO XCOLORS_BCAL, NColors=ncolors, Bottom=bottom, Title=title, File=file, $
...
registerName = 'XCOLORS_BCAL:' + title

   ; Only one XCOLORS_BCAL with this title.
...
IF !D.NAME NE 'MAC' THEN BEGIN
   draw = Widget_Draw(tlb, XSize=256, YSize=40, Expose_Events=0, $
      Retain=0, Event_Pro='XCOLORS_PROTECT_COLORS_BCAL')
ENDIF ELSE BEGIN
...
IF 1 - Keyword_Set(nosliders) THEN BEGIN
   ...
   botSlider = Widget_Slider(sliderbase, Value=0, Min=0, $
      Max=ncolors-1, XSize=256,Event_Pro='XCOLORS_BOTTOM_SLIDER_BCAL', $
      Title='Stretch Bottom', Drag=drag)
   topSlider = Widget_Slider(sliderbase, Value=ncolors-1, Min=0, $
      Max=ncolors-1, XSize=256, Event_Pro='XCOLORS_TOP_SLIDER_BCAL', $
   ...
   gammaSlider = Widget_Slider(sliderbase, Value=50.0, Min=0, Max=100, $
      Drag=drag, XSize=256, /Suppress_Value, Event_Pro='XCOLORS_GAMMA_SLIDER_BCAL', $
...
bar = XColors_Congrid_BCAL(bar, 256, 40, /INTERP)
WSet, windowIndex
TV, bar
...
XManager, registerName, tlb, Group=group, No_Block=noblock, Cleanup="XCOLORS_CLEANUP_BCAL"
