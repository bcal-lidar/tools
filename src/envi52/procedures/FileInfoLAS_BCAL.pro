...
envi_info_wid, report, title='Info: ' + file_basename(file), xs=max(strlen(report))+5, ys=40
    ...
    ; Check for negative values in x/y/z and show a warning window if found

negX = header.xMax lt 0 ? 1B : 0B
if negX eq 0B then negX = header.xMin lt 0 ? 1B : 0B

negY = header.yMax lt 0 ? 1B : 0B
if negY eq 0B then negY = header.yMin lt 0 ? 1B : 0B

negZ = header.zMax lt 0 ? 1B : 0B
if negZ eq 0B then negZ = header.zMin lt 0 ? 1B : 0B

if negX || negY || negZ then begin
  warnText = file_basename(file) + ' contains negative values in'
  if negX then warnText += ' X'
  if negY then warnText += ' Y'
  if negZ then warnText += ' Z'
  warnText += '.  Try Remove XYZ value before proceed.'
  okStr = DIALOG_MESSAGE(warnText)
endif
