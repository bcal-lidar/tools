...
products = {hmin    :{title:'Height: Minimum', points:1, index:-1, doIt:0}, $
            ...
            fhdElev    :{title:'Foliage Height Diversity (FHD) - All points', points:1, index:-1, doIt:0}, $
            fhdElev2    :{title:'Foliage Height Diversity (FHD) - Points above ground threshold', points:1, index:-1, doIt:0}}
...
gridBase = widget_auto_base(title='Vegetation metrics')
    ...
    fhdBase   = widget_base(leftBase, /row)
    dummy      = widget_param(fhdBase, default=1.0, prompt='Bin height for FHD: ', uvalue='fhdbin', /auto)
            ...
            if doPixel[i] then begin
                ...
                if needOne then begin
                    ...
                    while (index[0] eq -1L) do index = GetIndex_BCAL(i,j,iDim,jDim,arrayIndex,++factor)
                        ...
                        grndrt = where(height le grnt, grndcount, complement=abvgrnd, ncomplement=abvgrndct)
                        ...
                        if products.fhdElev.doIt then begin
                          fhd = 0d
                          for f=0d, ceil(maxht), fhdbin do begin
                            fhd_dummy = where(height ge f and height lt (f+fhdbin), fhd_c_n)
                            fhd_p = float(fhd_c_n)/float(nht)
                            if fhd_p gt 0 then fhd += - fhd_p * alog(fhd_p)
                          endfor
                          
                          dataArray[i,k,products.fhdElev.index]  = fhd
                          
                        endif
                        
                        if products.fhdElev2.doIt then begin
                          fhd2 = 0d
                          if abvgrndct then begin 
                            for f=0d, ceil(maxht), fhdbin do begin
                              fhd_dummy2 = where(height[abvgrnd] ge f and height[abvgrnd] lt (f+fhdbin), fhd_c_n2)
                              fhd_p2 = float(fhd_c_n2)/float(abvgrndct)
                              if fhd_p2 gt 0 then fhd2 += - fhd_p2 * alog(fhd_p2)
                            endfor
                          end

                          dataArray[i,k,products.fhdElev2.index]  = fhd2
