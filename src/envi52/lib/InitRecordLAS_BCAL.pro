...
; CALLING SEQUENCE:
;
;       record = InitRecordLAS(NODATA=NODATA, versionMajor=versionMajor, versionMinor=versionMinor)
;
; RETURN VALUE:
;
...
;       Added version keywords to handle LAS 1.4, April 2016
;
;###########################################################################
;
; LICENSE
...
function InitRecordLAS_BCAL, noData = noData, versionMajor=versionMajor, versionMinor=versionMinor
...
if (N_Elements(versionMajor) && N_Elements(versionMior)) then begin
  ...
  ; If LAS 1.4, define the variable length header structure differently
  ...
  if (versionMajor eq 1) && (versionMinor eq 4) then begin
      record = {formatR14,                                 $
                  ...
                  userID          : bytarr(16),           $ ; User ID
                  recordID        : 0US,                  $ ; Record ID
                  recordLength    : 0ULL,                  $ ; Record length after header
                  description     : bytarr(32)            $ ; Description
               }

      if ~ keyword_set(noData) then record = {formatR14n, inherits formatR14, data : ptr_new(/allocate)}    ; Data pointer
  endif
