...
; CALLING SEQUENCE:
;
;       projection = RecordsToProj_BCAL(records, reverse=reverse, versionMajor=versionMajor, versionMinor=versionMinor)
;
...
;       parameters are found, the function returns a value of !NULL.
;
...
;
;       Again, if no geographic parameters are found, the function returns a value of !NULL.
; DEPENDENCIES:
;
;       InitRecordLAS_BCAL
;       
...
;       Added version keywords to handle LAS 1.4, April 2015
;
;###########################################################################
;
; LICENSE
...
function RecordsToProj_BCAL, input, reverse=reverse, versionMajor=versionMajor, versionMinor=versionMinor
...
if keyword_set(reverse) then begin
    ...
    if n_tags(keys) then begin
        ...
        records = InitRecordLAS_BCAL(versionMajor=versionMajor, versionMinor=versionMinor)
    ...
    if dStart then begin
        ...
        dRecord = InitRecordLAS_BCAL(versionMajor=versionMajor, versionMinor=versionMinor)
    ...
    if aStart then begin
        ...
        aRecord = InitRecordLAS_BCAL(versionMajor=versionMajor, versionMinor=versionMinor)
    ...
    for a=0,n_elements(records)-1 do begin
        ...
        case records[a].recordID of
            ...
            34735 : begin
              ...
              if ptr_valid(records[a].data) then begin 
            ...
            34736 : begin
              ...
              if ptr_valid(records[a].data) then begin
                doubleParams = double((*records[a].data), 0, records[a].recordLength/8)
              endif
